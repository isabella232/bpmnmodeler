 ...
 * @param s the string
 */
public TextFlowHacked(String s) {
	text = s;
}
...
boolean addLeadingWordWidth(String text, int[] width) {
	...
	text = 'a' + text + 'a';
	FlowUtilitiesHacked.LINE_BREAK.setText(text);
	int index = FlowUtilitiesHacked.LINE_BREAK.next() - 1;
	if (index == 0)
		return true;
	...
	if (bidiInfo == null)
		width[0] += getTextUtilitiesHacked().getStringExtents(text, getFont()).width;
	else {
		TextLayout textLayout = FlowUtilitiesHacked.getTextLayout();
...
private int findOffset(Point p, int[] trailing, TextFragmentBox box, int boxIndex) {
	if (box == null)
		return -1;
	TextLayout layout = FlowUtilitiesHacked.getTextLayout();
...
int getAscent() {
    return getTextUtilitiesHacked().getAscent(getFont());
}
...
Point getPointInBox(TextFragmentBox box, int offset, int index, boolean trailing) {
	...
	if (bidiInfo == null) {
		...
		result.x = getTextUtilitiesHacked().getStringExtents(substring, getFont()).width;
	} else {
		TextLayout layout = FlowUtilitiesHacked.getTextLayout();
		layout.setFont(getFont());
...
int getDescent() {
    return getTextUtilitiesHacked().getDescent(getFont());
}
...
public boolean isTextTruncated() {
	for (int i = 0; i < getFragments().size(); i++) {
		if (((TextFragmentBox)getFragments().get(i)).isTruncated())
			return true;
	}
	return false;
}
...
protected void paintText(Graphics g, String draw, int x, int y, int bidiLevel) {
	if (bidiLevel == -1) {
		...
		TextLayout tl = FlowUtilitiesHacked.getTextLayout();
		if (isMirrored())
 ...
 * @since 3.4
 */
protected FlowUtilitiesHacked getFlowUtilitiesHacked() {
    return FlowUtilitiesHacked.INSTANCE;
}
 ...
 * @since 3.4
 */
protected TextUtilitiesHacked getTextUtilitiesHacked() {
    return TextUtilitiesHacked.INSTANCE;
}
