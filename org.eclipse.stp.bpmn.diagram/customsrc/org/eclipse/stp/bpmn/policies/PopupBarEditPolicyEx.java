 ...
 * <p>
 * This code is derived from internal classes of GMF. The essential differences:
 * The height is different to accomodate icons bigger than 16. The layout is
 * different to put the tools on 2 lines instead of a single line The
 * create-request will be made at the location where the cusor was when the
 * dialog appeared instead of where the cursor is when it is clicked. The
 * balloon is positionned to try to avoid clipping in the editor window.
 * </p>
 * 
 * @author MPeleshchyshyn
 */
public class PopupBarEditPolicyEx extends DiagramAssistantEditPolicy {
    ...
    /**
     * Constraints the popup bar to appear at a fixed y coordionates.
     * Used for the sub-process border.
     * @param thisPart
     */
...
//        setIsDisplayAtMouseHoverLocation(true);
    ...
    /**
     * The reference point copy last time getBalloonPosition was called.
     */
...
//                if (getHost() instanceof BpmnDiagramEditPart) {
//                    System.err.println("calling mouseover for diagram");
//                    mouseHover(me);
//                    return;
//                }
    ...
    /** Y postion offset from shape where the balloon top begin. */
    static private int ACTION_HGT = 35;

    static private int ACTION_WIDTH = 30;
    ...
    protected void addPopupBarDescriptor(IElementType elementType,
        ...
        this.addPopupBarDescriptor(elementType, theImage, new PopupBarToolEx(
                this, getHost(), elementType));
    ...
    protected void addPopupBarDescriptor(IElementType elementType,
        ...
        PopupBarToolEx theTracker = new PopupBarToolEx(this, getHost(),
                elementType);
        PopupBarDescriptor desc = new PopupBarDescriptor(theTip, theImage,
                elementType, theTracker);
        myPopupBarDescriptors.add(desc);
    ...
    /**
     * @author hmalphettes makes sure the element created is created where the
     *         cursor was when the popup appeared. Not where it was clicked.
     * 
     * @author <a href="http://www.intalio.com">&copy; Intalio, Inc.</a>
     * 
     */
    static final class PopupBarToolEx extends PopupBarTool {
        private final PopupBarEditPolicyEx _editPolicy;

        public PopupBarToolEx(PopupBarEditPolicyEx editPolicy, EditPart epHost,
        ...
        /**
         * Derived from the super. Tweaked the command location.
         * 
        ...
        protected Command getCommand() {
            ...
            if (theRequest instanceof CreateRequest
                    && _editPolicy._lastRePointForBalloonPosition != null) {
                ((CreateRequest) theRequest)
                        .setLocation(_editPolicy._lastRePointForBalloonPosition);
            } else {
    ...
    private void initPopupBars() {
        ...
        if (theList.size() > 4) {
            ...
            // ACTION_WIDTH * theList.size() + ACTION_MARGIN_RIGHT;

            getBalloon().setSize(iTotal,
                    ACTION_HGT * 2 + 2 * ACTION_BUTTON_START_Y);
            // ACTION_HGT + 2 * ACTION_BUTTON_START_Y);

        } else {
            int iTotal = ACTION_WIDTH * theList.size() + ACTION_MARGIN_RIGHT;
            getBalloon()
                    .setSize(iTotal, ACTION_HGT + 2 * ACTION_BUTTON_START_Y);
        }

        int xLoc = ACTION_BUTTON_START_X;
        int yLoc = ACTION_BUTTON_START_Y;

        int halphIdx = theList.size() > 4 ? theList.size() / 2 + theList.size()
                % 2 - 1 : -1;
        for (int idx = 0; idx < theList.size(); idx++) {
    ...
    public void showDiagramAssistant() {
    ...
    protected void showDiagramAssistant(Point referencePoint) {
        ...
        if (_thisPart != null) {
            Point center = _thisPart.getFigure().getBounds().getCenter();
    ...
    private Point getBalloonPosition(Point referencePoint) {
        _lastRePointForBalloonPosition = referencePoint.getCopy();
        ((GraphicalEditPart)getHost()).getFigure()
            .translateToAbsolute(_lastRePointForBalloonPosition);
        
        Point thePoint = new Point();
        boolean atMouse = getIsDisplayAtMouseHoverLocation();
        int descriptorsCount = getPopupBarDescriptors().size();
        if (atMouse) {
            ...
            if (willBalloonBeClipped(thePoint)) {
                ...
                if (ctrl1 instanceof FigureCanvas) {
                    ...
                    if (thePoint.x + dim.width >= vpRect.x + vpRect.width) {
                        thePoint.x = Math.max(vpRect.right() - dim.width, Math
                                .max(vpRect.x, rcBounds.x));
                    }
                }
            }
            ...
            if (_thisPart != null) {
                //center things and then move to the left of the sub-process handle.
                theoffset.height += rcBounds.height/2;
                thePoint.y = rcBounds.y + theoffset.height;
                thePoint.x = rcBounds.x + rcBounds.width/2 + 32;
                    //the width of the marker:
//                    MapModeUtil.getMapMode(getHostFigure()).LPtoDP(32);
//                    -getBalloon().getBounds().width/3;
            } else {
        ...
        getHostFigure().translateToRelative(refPointCopy);
        getBalloon().translateToRelative(refPointCopy);
