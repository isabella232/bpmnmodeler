 ...
 * Bad surprise how many things need to be overridden 
 * before we can start doing things.
 * <p>
 * Scary: copied all necessarry code to not refer to static methods
 * Only extending the ObliqueRouter
 * </p>
 * <p>
 ...
 * center-strategy: we try to introduce bendpoints in the bary center of
 * the segment decomposed</li>
 * <li>The anchors are not assumed to be slideable: not "adjust to middle"
 *  or abusive "make it a linear bi-terminal."</li>
...
public class RectilinearRouterEx extends ObliqueRouter implements OrthogonalRouter {
    ...
    /** appropriate to make
     * segments horizontal when next to the anchor
     * and introduce points in the middle. */
    public static final int NORMALIZE_ON_VERTICAL_CENTER = 2;
    
    /** appropriate to make
     * segments vertical when next to the anchor
     * and introduce points in the middle. */
    public static final int NORMALIZE_ON_HORIZONTAL_CENTER = 3;
    
    /**
     * Strategy used when routing a sequence edge that starts on a shape on
     * the sub-process border.
     */
...
//            }
    ...
    protected boolean removePointsInViews(Connection conn, PointList newLine) {
        ...
        if (conn == null || newLine == null ) {
            throw new IllegalArgumentException("null connection illegal"); //$NON-NLS-1$
        }
        ...
        Rectangle startRect = new Rectangle(getOwnerBounds(conn.getSourceAnchor()));
        conn.getSourceAnchor().getOwner().translateToAbsolute(startRect);
        conn.translateToRelative(startRect);

        Rectangle endRect = new Rectangle(getOwnerBounds(conn.getTargetAnchor()));
    ...
    protected boolean removeSegmentsInViews(Connection conn, PointList newLine) {
        ...
        Rectangle startRect =
            new Rectangle(getOwnerBounds(conn.getSourceAnchor()));
        conn.getSourceAnchor().getOwner().translateToAbsolute(startRect);
        conn.translateToRelative(startRect);
        ...
        Rectangle endRect =
            new Rectangle(getOwnerBounds(conn.getTargetAnchor()));
        conn.getTargetAnchor().getOwner().translateToAbsolute(endRect);
        conn.translateToRelative(endRect);
...
//        }
    ...
    /**
     * When comparing things with absolute coordinates,
     * 2 numbers that have just one digit of difference can be assumed to be
     * because of previous approximations: they should really be considered equals.
     * This makes sure that when we compare them they are recognized as zero.
     * @param rawCompute
     * @return
     */
    ...
    /**
     * It 2 numbers are grossly equal, there is not enough distance
     * between them to insert a mid-point that would not be almostEqual 
     * to both points. 
     * @param rawCompute
     * @return true when they are too close to insert a point
     * in between them that will be not almost equal to either of them.
     */
     ...
     * @param pointList
     */
    public void routeLine(Connection conn,
            int nestedRoutingDepth, PointList pointList) {
...
//        int currentPointListLength = pointList.size();
        ...
        normalizationChangedLine |= PointListUtilities.normalizeSegments(pointList, tolerance);
...
//      resetEndPointsToEdge(conn, pointList); 
    ...
    private void route2Points(Point oldPointA,
        ...
        case NORMALIZE_ON_HORIZONTAL_CENTER:
            ...
            if (isFirst || isLast || (
                    !areAlmostEqual(oldPointA.y, oldPointB.y) && 
...
//                break;//it is perfect don't touch it.
            ...
            if (isFirst || isLast || ((!areAlmostEqual(oldPointA.y, oldPointB.y) &&
                ...
                if ((isFirst || isLast)
                    ...
                    int signedMinDistFromSide = oldPointB.y > oldPointA.y ?
                            minDistFromSide : - minDistFromSide;
                    if (isFirst) {
    ...
    /**
     * Called for closest distance.
     * Applies a custom algorithm for sequence edge to make sure that
     * if the connection goes from right to left it goes around the
            ...
            if (ptOrig.x >= ptTerm.x 
                ...
                if (NORMALIZE_ON_HORIZONTAL_EXCEPT_FIRST_SEG != normalizeBehavior
                    ...
                    if (NORMALIZE_ON_HORIZONTAL_EXCEPT_FIRST_SEG != normalizeBehavior) {
                        ...
                        int aboveDelta = Math.max(ptOrig.y - aboveYone, ptTerm.y - aboveYone);
                        int belowDelta = Math.max(belowYone - ptOrig.y, belowYone - ptTerm.y);
                        
                        if (aboveDelta - belowDelta > tolerance) {
                            //consider that one of them really is a shorter path:
                            aboveY = belowYone + extraY;
                            ...
                            if (ptOrig.y + tolerance > midYSrc) {//+tolerance as we favor going above
                                aboveY = belowYone + extraY;
                            } else {
                                aboveY = aboveYone - extraY;
                            }
                ...
                if (NORMALIZE_ON_HORIZONTAL_EXCEPT_FIRST_SEG == normalizeBehavior) {
                    //make sure we are far enough on the left
                    //to avoid the sub-process on which the src shape is
                    //anchored on.
                ...
                //let's make sure that we don't need to route around the sub-process
