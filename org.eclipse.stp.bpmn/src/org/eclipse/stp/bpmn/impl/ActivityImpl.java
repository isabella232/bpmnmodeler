...
public class ActivityImpl extends VertexImpl implements Activity {
		...
		if (newEventHandlerFor != eInternalContainer() || (eContainerFeatureID != BpmnPackage.ACTIVITY__EVENT_HANDLER_FOR && newEventHandlerFor != null)) {
			if (EcoreUtil.isAncestor(this, newEventHandlerFor))
				throw new IllegalArgumentException(BpmnMessages.bind(
						BpmnMessages.ActivityImpl_recursiveContainment,
						toString()));
	...
	public String toString() {
		...
		result.append(BpmnMessages.ActivityImpl_doc);
		result.append(documentation);
		result.append(BpmnMessages.ActivityImpl_name);
		result.append(name);
		result.append(BpmnMessages.ActivityImpl_ncname);
		result.append(ncname);
		result.append(BpmnMessages.ActivityImpl_orderedMessages);
		result.append(orderedMessages);
		result.append(BpmnMessages.ActivityImpl_activityType);
		if (activityTypeESet) result.append(activityType); else result.append(BpmnMessages.ActivityImpl_unset);
		result.append(BpmnMessages.ActivityImpl_looping);
		if (loopingESet) result.append(looping); else result.append(BpmnMessages.ActivityImpl_unset);
		result.append(')');
